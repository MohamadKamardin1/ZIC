...
import { gcClaims } from "../../lib/gc-api"
...
export default function GCClaims() {
  ...
  const loadClaims = useCallback(async () => {
    ...
    try {
      ...
      const res = await gcClaims.list(params)
  ...
  async function viewDetail(id: string) {
    ...
    try {
      const detail = await gcClaims.get(id)
  ...
  async function handleAction(action: string, id: string) {
    ...
    try {
      if (action === "assess") await gcClaims.assess(id, {})
      else if (action === "approve") await gcClaims.approve(id, {})
      else if (action === "reject") await gcClaims.reject(id, "Rejected by claims officer")
      else if (action === "pay") await gcClaims.pay(id, selectedClaim?.approvedAmount || 0)
      await viewDetail(id)
      await loadClaims()
    } catch (err: any) { alert(err.message) }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-muted-foreground mt-1">Group Credit claims registration & processing</p>
      </div>
