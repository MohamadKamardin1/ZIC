...
import { gcMedicalCases, gcSetup } from "../../lib/gc-api"
...
export default function GCMedicalUW() {
  ...
  const loadCases = useCallback(async () => {
    ...
    try {
      ...
      const res = await gcMedicalCases.list(params)
  ...
  useEffect(() => {
    gcSetup.listUWDecisions().then((res: any) => setDecisions(res?.results ?? res?.data ?? res ?? [])).catch(() => {})
  }, [])

  async function viewDetail(id: string) {
    setDetailLoading(true)
    try {
      const detail = await gcMedicalCases.get(id)
  ...
  async function handleDecision() {
    ...
    setSaving(true)
    try {
      await gcMedicalCases.makeDecision(selectedCase.id, decisionForm)
