...
import { FileText, Plus, Search, Edit2, Trash2, X, Loader2, AlertCircle, ShieldCheck } from "lucide-react";
...
export default function OLClaims() {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const data = await olCore.listClaims();
      setItems(data);
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        await olCore.updateClaim(editingItem.id, formData);
      } else {
        await olCore.createClaim(formData);
      }
      closeModal();
      fetchItems();
    } catch (err: any) {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="mb-6 flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage Ordinary Life Claims</p>
        </div>
        <button
          onClick={() => openModal()}
        ...
        >
          <Plus className="h-4 w-4" /> Add Claims
        </button>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-foreground">
            <thead className="bg-secondary/30 text-muted-foreground border-b border-border">
              <tr>
                <th className="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Claim No.</th>
                <th className="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Policy ID</th>
                <th className="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Date of Event</th>
                <th className="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Cause</th>
                <th className="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Claim Amount</th>
              ...
              {loading ? (
                ...
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-secondary/20 transition-colors">
                    <td className="px-6 py-4">{item.claim_number}</td>
                    <td className="px-6 py-4">{item.policy}</td>
                    <td className="px-6 py-4">{item.date_of_event}</td>
                    <td className="px-6 py-4">{item.cause}</td>
                    <td className="px-6 py-4">{item.claim_amount}</td>
          ...
          <div className="bg-card border border-border rounded-2xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95">
            ...
            <div className="p-6 overflow-y-auto">
              <form id="record-form" onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Claim No.</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Policy ID</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Date of Event</label>
                  <input
                    type="date"
                    required
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Cause</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Claim Amount</label>
                  <input
                    type="number"
                    required
                    value={formData.claim_amount || ''}
                    onChange={e => setFormData({ ...formData, claim_amount: Number(e.target.value) })}
                    className="w-full h-10 bg-card border border-border rounded-xl px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/40"
                  />
                </div>
                <div>
