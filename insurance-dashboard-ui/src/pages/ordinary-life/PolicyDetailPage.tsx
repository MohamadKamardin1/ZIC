...
import { usePolicyDetail, usePolicyOptions } from "../../lib/policiesHooks"
import type { PolicyAuditEntry, PolicyDetail } from "../../lib/policies"
...
function MembersPreview({ policy }: { policy: PolicyDetail }) {
  return <section className="surface-card p-4" aria-labelledby="composition-heading"><div className="mb-4 flex items-center justify-between gap-3"><div><h2 id="composition-heading" className="text-sm font-bold">Covered lives and riders</h2><p className="mt-1 text-xs text-[var(--muted-foreground)]">Read-only policy composition; servicing actions will be added in the Members & Riders prompt.</p></div><ShieldCheck size={18} className="text-[var(--success)]" aria-hidden="true" /></div><div className="overflow-x-auto"><table className="w-full min-w-[680px] text-left text-sm"><thead><tr className="border-b text-xs uppercase tracking-[0.08em] text-[var(--muted-foreground)]"><th className="px-3 py-2">Name</th><th className="px-3 py-2">Relation</th><th className="px-3 py-2">DOB</th><th className="px-3 py-2">Gender</th><th className="px-3 py-2 text-right">Cover</th></tr></thead><tbody>{policy.members.map((member) => <tr key={member.id} className="border-b last:border-0"><td className="px-3 py-3 font-semibold">{renderFk(member.name)}</td><td className="px-3 py-3">{renderFk(member.memberRelation)}</td><td className="px-3 py-3">{dateLabel(member.dob)}</td><td className="px-3 py-3">{renderFk(member.gender)}</td><td className="px-3 py-3 text-right"><MoneyCell value={member.benefitAmount} currency={policy.currency} /></td></tr>)}</tbody></table></div>{policy.riders.length > 0 && <div className="mt-4 flex flex-wrap gap-2">{policy.riders.map((rider) => <StatusBadge key={rider.id} value={renderFk(rider.riderCode)} tone="info" />)}</div>}</section>
}
...
export default function PolicyDetailPage() {
  ...
  const actionSlot = <div className="flex flex-wrap justify-end gap-2">{canAction("endorse") ? <DetailAction label="Endorse" icon={<FileText size={14} aria-hidden="true" />} onClick={() => goAction("endorse")} /> : null}{canAction("loan") ? <DetailAction label="Loan" icon={<Banknote size={14} aria-hidden="true" />} onClick={() => goAction("loan")} /> : null}{canAction("withdraw") ? <DetailAction label="Withdraw" icon={<HandCoins size={14} aria-hidden="true" />} onClick={() => goAction("withdraw")} /> : null}{canAction("surrender") ? <DetailAction label="Surrender" icon={<Landmark size={14} aria-hidden="true" />} onClick={() => goAction("surrender")} /> : null}{canAction("print") ? <DetailAction label="Print" icon={<FileText size={14} aria-hidden="true" />} onClick={() => goAction("print")} /> : null}</div>
  ...
  return <MasterDetailPage eyebrow="Ordinary Life · Policy detail" title={policy.policyNumber} description="Read-only contract overview sourced from the immutable issuance snapshot." status={{ value: policy.statusDisplay, tone: lapsed ? "danger" : matured ? "success" : "info" }} actions={<button type="button" className="button-secondary border-white/30 bg-white/10 text-white hover:bg-white/20" onClick={() => navigate("/ordinary-life/policies")}><ArrowLeft size={15} aria-hidden="true" />Back to policies</button>}>
    ...
    {activeTab === "members" && <MembersPreview policy={policy} />}
    {activeTab === "endorsements" && <InfoBanner title="Endorsements">The endorsement history and creation workflow will appear here without changing the issued snapshot directly.</InfoBanner>}
    {activeTab === "financials" && <InfoBanner title="Financials">Loans, withdrawals, surrender values, and maturity records will appear here in the servicing prompts.</InfoBanner>}
