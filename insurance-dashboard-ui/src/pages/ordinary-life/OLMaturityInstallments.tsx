...
export default function OLMaturityInstallments() {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const data = await olCore.listMaturityInstallments();
      setItems(data);
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        await olCore.updateMaturityInstallment(editingItem.id, formData);
      } else {
        await olCore.createMaturityInstallment(formData);
      }
      closeModal();
      fetchItems();
    } catch (err: any) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await olCore.deleteMaturityInstallment(id);
      fetchItems();
    } catch (err: any) {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="mb-6 flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage Ordinary Life Maturity Installments</p>
        </div>
        <button
          onClick={() => openModal()}
        ...
        >
          <Plus className="h-4 w-4" /> Add Maturity Installments
        </button>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-foreground">
            <thead className="bg-secondary/30 text-muted-foreground border-b border-border">
              <tr>
                <th className="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Installment No.</th>
              ...
              {loading ? (
                ...
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-secondary/20 transition-colors">
                    <td className="px-6 py-4">{item.installment_number}</td>
          ...
          <div className="bg-card border border-border rounded-2xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95">
            <div className="px-6 py-4 border-b border-border flex justify-between items-center bg-card">
              <h3 className="text-lg font-bold text-foreground">
                {editingItem ? 'Edit' : 'Add'} Maturity Installments
              </h3>
            ...
            <div className="p-6 overflow-y-auto">
              <form id="record-form" onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Installment No.</label>
                  <input
                    type="text"
                    required
