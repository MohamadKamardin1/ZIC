...
const valueLabel = (value: unknown) => value === null || value === undefined || value === "" ? "—" : typeof value === "object" ? JSON.stringify(sanitizeForDisplay(value)) : renderFk(value)
const normalizeParameterRecord = (raw: ParameterRecord & Record<string, unknown>): ParameterRecord => ({
  ...
  partner_display: (raw.partner_display ?? raw.partnerDisplay) as string | null | undefined,
  product_display: (raw.product_display ?? raw.productDisplay) as string | null | undefined,
  plan_display: (raw.plan_display ?? raw.planDisplay) as string | null | undefined,
...
const screens: Record<ScreenKey, ScreenConfig> = {
  ...
  commission: {
    ...
    permission: "ol_parameters.update",
    columns: commonColumns([
      { key: "partner", label: "Partner / agent", field: "partner", sortable: true, render: (value, row) => renderFk(value, row.partner_display) },
      { key: "product", label: "Product", field: "product", sortable: true, render: (value, row) => renderFk(value, row.product_display) },
      { key: "plan", label: "Plan", field: "plan", sortable: true, render: (value, row) => renderFk(value, row.plan_display) },
  ...
  maturity: {
    ...
    permission: "ol_parameters.update",
    columns: commonColumns([
      { key: "product", label: "Product scope", field: "product", sortable: true, render: (value, row) => renderFk(value, row.product_display) },
      { key: "plan", label: "Plan scope", field: "plan", sortable: true, render: (value, row) => renderFk(value, row.plan_display) },
...
function Editor({ screen, value, onChange, overlapWarning }: { screen: ScreenKey; value: EditorState; onChange: (key: string, next: string | number | boolean | null) => void; overlapWarning?: string }) {
  ...
  if (screen === "commission") return <div className="space-y-4">{common}<InfoBanner title="Overlap warning">{overlapWarning ?? "The backend checks overlapping effective periods for matching scope during save."}</InfoBanner><div className="grid gap-4 sm:grid-cols-2"><SmartSelect entity="agents" label="Partner / agent" name="partner" value={String(field("partner") ?? "")} onChange={(next) => onChange("partner", next)} /><SmartSelect entity="products" label="Product" name="product" value={String(field("product") ?? "")} onChange={(next) => onChange("product", next)} /><div className="space-y-1"><span className="text-sm font-medium">Plan</span><div className="flex h-10 items-center rounded-[10px] border bg-[var(--muted)]/30 px-3 text-sm">{renderFk(field("plan"), field("plan_display"))}</div><p className="text-xs text-[var(--muted-foreground)]">Plan selection is preserved from the backend scope.</p></div><TextField label="Channel" name="channel" value={String(field("channel") ?? "")} onChange={(event) => onChange("channel", event.target.value)} /><SelectField label="Rate type" name="rate_type" required value={String(field("rate_type") ?? "PERCENTAGE")} options={RATE_TYPES} onChange={(event) => onChange("rate_type", event.target.value)} /><DecimalField label="Rate value" name="rate_value" required value={String(field("rate_value") ?? "")} onChange={(event) => onChange("rate_value", event.target.value)} error={error("rate_value")} /><TextField label="Priority" name="priority" type="number" value={String(field("priority") ?? 100)} onChange={(event) => onChange("priority", Number(event.target.value))} /><TextAreaField label="Reason / narration" name="reason" value={String(field("reason") ?? "")} onChange={(event) => onChange("reason", event.target.value)} /></div></div>
