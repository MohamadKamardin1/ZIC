...
export default function OLLoans() {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const data = await olCore.listLoans();
      setItems(data);
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        await olCore.updateLoan(editingItem.id, formData);
      } else {
        await olCore.createLoan(formData);
      }
      closeModal();
      fetchItems();
    } catch (err: any) {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="mb-6 flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage Ordinary Life Loans</p>
        </div>
        <button
          onClick={() => openModal()}
        ...
        >
          <Plus className="h-4 w-4" /> Add Loans
        </button>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-foreground">
            <thead className="bg-secondary/30 text-muted-foreground border-b border-border">
              <tr>
                <th className="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Loan No.</th>
                <th className="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Policy ID</th>
                <th className="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Loan Amount</th>
                <th className="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Interest Rate %</th>
                <th className="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Outstanding Balance</th>
              ...
              {loading ? (
                ...
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-secondary/20 transition-colors">
                    <td className="px-6 py-4">{item.loan_number}</td>
                    <td className="px-6 py-4">{item.policy}</td>
                    <td className="px-6 py-4">{item.loan_amount}</td>
                    <td className="px-6 py-4">{item.interest_rate}</td>
                    <td className="px-6 py-4">{item.outstanding_balance}</td>
          ...
          <div className="bg-card border border-border rounded-2xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95">
            ...
            <div className="p-6 overflow-y-auto">
              <form id="record-form" onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Loan No.</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Outstanding Balance</label>
                  <input
                    type="number"
                    required
