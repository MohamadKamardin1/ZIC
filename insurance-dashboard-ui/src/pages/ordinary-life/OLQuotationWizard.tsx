...
type FundAllocation = {
  id?: string
  /** Canonical field accepted by POST /investment-funds/. */
  plan_config_id?: string
  /** Read shape returned by the allocation snapshot endpoint. */
  plan_configuration_id?: string
  fund_id: string
...
function resolveBenefitTypeId(value: unknown, options: Array<Record<string, unknown>> = []): string | null {
  const candidate = String(value ?? "").trim()
  if (!candidate) return null
  if (UUID_PATTERN.test(candidate)) return candidate
  const match = options.find((option) => [option.value, option.id, option.code, option.label, option.name].some((item) => String(item ?? "").trim().toLowerCase() === candidate.toLowerCase()))
  const resolved = match?.value ?? match?.id
  return UUID_PATTERN.test(String(resolved ?? "")) ? String(resolved) : null
}
...
function ChoiceSelect({ label, name, value, options, required, error, onChange, placeholder = "Select an option", disabled = false }: { label: string; name: string; value: string; options: Choice[]; required?: boolean; error?: string; onChange: (value: string) => void; placeholder?: string; disabled?: boolean }) {
  return <SelectInput label={label} name={name} required={required} error={error} value={value} disabled={disabled} onChange={(event) => onChange(event.target.value)}>
    <option value="">{placeholder}</option>
    {options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
  </SelectInput>
}

const YES_NO_OPTIONS: Choice[] = [{ value: "false", label: "No" }, { value: "true", label: "Yes" }]

function YesNoSelect({ label, name, value, required, disabled, onChange }: { label: string; name: string; value: boolean; required?: boolean; disabled?: boolean; onChange: (value: boolean) => void }) {
  return <ChoiceSelect label={label} name={name} required={required} value={String(Boolean(value))} options={YES_NO_OPTIONS} disabled={disabled} onChange={(next) => onChange(next === "true")} placeholder="Select" />
}
...
function PlanSelectionPanel({ plans, selectedPlanIds, search, loading, disabled = false, onSearch, onToggle, onProductCreated }: { plans: PlanCard[]; selectedPlanIds: string[]; search: string; loading: boolean; disabled?: boolean; onSearch: (value: string) => void; onToggle: (plan: PlanCard) => void; onProductCreated: (option: QuickCreateOption) => void | Promise<void> }) {
  ...
  return <aside className="w-full shrink-0 lg:w-[310px] xl:w-[330px]" aria-label="Plan selection panel">
    <div className="surface-card ol-quote-plan-panel overflow-hidden">
      ...
      <div className="max-h-[calc(100vh-250px)] space-y-1 overflow-auto bg-white p-2.5">
        ...
        {!loading && plans.map((plan) => {
          ...
          return <button key={`${plan.product_version_id}-${plan.plan_id}`} type="button" disabled={disabled} aria-pressed={selected} onClick={() => onToggle(plan)} className={`w-full rounded-[8px] border p-3 text-left transition disabled:cursor-not-allowed disabled:opacity-70 ${selected ? "border-[var(--ol-indigo)] bg-[#f8f8ff] shadow-sm" : "border-transparent bg-white hover:border-[#d8d9ee] hover:bg-[#fbfbfd]"}`}>
            <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="mb-1 flex flex-wrap items-center gap-1.5"><span className={`rounded-[4px] px-1.5 py-0.5 text-[10px] font-bold tracking-[0.08em] ${selected ? "bg-[#6562cd] text-white" : "bg-[#f0f1f5] text-[#555b69]"}`}>{plan.code}</span><span className="text-[10px] text-[#a0a4ad]">Plan Type</span>{badges.map((badge) => <span key={badge} className={`rounded-[4px] px-1.5 py-0.5 text-[10px] font-semibold ${badge.toLowerCase().includes("joint") ? "bg-[#d9effc] text-[#2182bd]" : "bg-[#d8f4df] text-[#2b9b55]"}`}>{planBadgeLabel(badge)}</span>)}</div><h3 className="truncate text-sm font-bold text-[#30333b]">{plan.name}</h3><p className="mt-1 line-clamp-2 text-[11px] leading-4 text-[#a0a4ad]">{plan.description || plan.product_name || "Configured Ordinary Life plan"}</p></div><span className={`mt-1 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border ${selected ? "border-[var(--ol-indigo)] bg-[var(--ol-indigo)] text-white" : "border-[#d9dce5] text-transparent"}`}>{selected && <Check size={13} aria-hidden="true" />}</span></div>
          </button>
        })}
      </div>
      {canCreateProducts && !disabled && <QuickCreateModal open={quickCreateOpen} entity="products" entityLabel="Product" permissionCode="ol_parameters.create" onClose={() => setQuickCreateOpen(false)} onCreated={(option) => { setQuickCreateOpen(false); return onProductCreated(option) }} />}
    </div>
  </aside>
}
...
function WizardTabs({ activeStep, completedSteps, invalidSteps, onSelect }: { activeStep: number; completedSteps: Set<number>; invalidSteps: Set<number>; onSelect: (index: number) => void }) {
  return <nav className="ol-quote-tabs overflow-hidden rounded-[10px]" aria-label="Quotation wizard steps"><ol className="flex flex-wrap items-center gap-0.5 p-1.5">{steps.map((step, index) => { const Icon = step.icon; const active = activeStep === index; const completed = completedSteps.has(index); const invalid = invalidSteps.has(index); return <li key={step.id} className="flex shrink-0 items-center"><button type="button" aria-current={active ? "step" : undefined} data-active={active} onClick={() => onSelect(index)} className={`ol-quote-tab flex items-center gap-1.5 rounded-[7px] px-2.5 py-2 text-left text-[11px] font-bold transition md:px-3 md:text-xs ${invalid ? "text-[var(--destructive)]" : ""}`}><span className={`ol-quote-tab-icon flex h-6 w-6 items-center justify-center rounded-full border ${completed && !active ? "border-[var(--success)] text-[var(--success)]" : invalid ? "border-[var(--destructive)] text-[var(--destructive)]" : "border-current/20"}`}>{completed && !active ? <Check size={13} aria-hidden="true" /> : invalid ? <CircleAlert size={13} aria-hidden="true" /> : <Icon size={13} aria-hidden="true" />}</span><span className="whitespace-nowrap">{step.label}</span></button>{index < steps.length - 1 && <span className="mx-0.5 hidden h-px w-2 bg-[var(--border)] 2xl:block" aria-hidden="true" />}</li> })}</ol></nav>
}
...
function PersonalDetailsStep({ form, options, errors, age, onChange }: { form: PersonalForm; options: { identityTypes: Choice[]; genders: Choice[]; smokerStatuses: Choice[]; locations: Choice[]; agents: Choice[] }; errors: ApiFieldErrors; age: number | null; onChange: (field: keyof PersonalForm, value: string) => void }) {
  return <div className="surface-card ol-quote-surface overflow-visible"><h2 className="sr-only">Personal Details</h2><div className="border-b px-5 py-3"><p className="text-[11px] font-semibold text-[#a3a6b3]">Personal Details</p></div><div className="space-y-5 p-5"><FormGrid columns={3}>
    ...
    <SmartSelect entity="identity-types" label="Identity Type" name="identity_type" required value={form.identity_type} onChange={(value) => onChange("identity_type", value)} error={fieldError(errors, "identity_type")} placeholder="Select identity type" />
    ...
    <DateInput label="Date of Birth" name="date_of_birth" required value={form.date_of_birth} onChange={(event) => onChange("date_of_birth", event.target.value)} error={fieldError(errors, "date_of_birth")} placeholder="dd/mm/yyyy" />
    <ReadOnlyField label="Age" value={age === null ? "Computed" : `${age} years`} />
    <ChoiceSelect label="Gender" name="gender" required value={form.gender} options={options.genders} onChange={(value) => onChange("gender", value)} error={fieldError(errors, "gender")} placeholder="Select gender" />
    <ChoiceSelect label="Smoker" name="smoker_status" required value={form.smoker_status} options={options.smokerStatuses} onChange={(value) => onChange("smoker_status", value)} error={fieldError(errors, "smoker_status")} placeholder="Select smoker status" />
    <SmartSelect entity="locations" label="Location" name="location_id" required value={form.location_id} onChange={(value) => onChange("location_id", value)} error={fieldError(errors, "location_id") ?? fieldError(errors, "location")} placeholder="Search for a location..." manageHref="/system-parameters/partner/locations" />
    <SmartSelect entity="agents" label="Agent" name="agent_id" required value={form.agent_id} onChange={(value) => onChange("agent_id", value)} error={fieldError(errors, "agent_id")} placeholder="Select agent" />
  </FormGrid>
  <TextareaInput label="Address" name="address" required value={form.address} onChange={(event) => onChange("address", event.target.value)} error={fieldError(errors, "address")} placeholder="Enter full address" />
  </div></div>
}
...
function PlanConfigurationSection({ index, config, card, options, errors, onChange }: { index: number; config: PlanConfiguration; card?: PlanCard; options: PlanOptions; errors: ApiFieldErrors; onChange: (field: string, value: string | boolean) => void }) {
  ...
  <FormGrid columns={3}><YesNoSelect label="Joint Life" name={`joint_life_${config.id}`} required value={Boolean(config.joint_life)} disabled={!features.joint_life} onChange={(checked) => onChange("joint_life", checked)} /><YesNoSelect label="Mortgage" name={`mortgage_${config.id}`} required value={Boolean(config.mortgage)} disabled={!features.mortgage} onChange={(checked) => onChange("mortgage", checked)} /><YesNoSelect label="Personal Accident (PA)" name={`personal_accident_${config.id}`} required value={Boolean(config.personal_accident)} disabled={!features.personal_accident} onChange={(checked) => onChange("personal_accident", checked)} /><YesNoSelect label="Premium Waiver (WP)" name={`premium_waiver_${config.id}`} required value={Boolean(config.premium_waiver)} disabled={!features.premium_waiver} onChange={(checked) => onChange("premium_waiver", checked)} /></FormGrid>
  </div></section>
}
...
function ConfigureInstallmentModal({ open, plan, template, loading, saving, errors, onClose, onSave }: { open: boolean; plan: InstallmentPlanRow | null; template: InstallmentTemplate | null; loading: boolean; saving: boolean; errors: ApiFieldErrors; onClose: () => void; onSave: (payload: { annuity_period_years: number; payment_mode: string; after_maturity_benefits: boolean; before_maturity_benefits: boolean; rate_rows: InstallmentRateRow[] }) => Promise<boolean> }) {
  ...
  return <Modal open={open} title="Configure Installments" description={plan ? `Plan/Sub-Product: ${plan.plan_name}` : undefined} onClose={onClose} size="xl" footer={<><button type="button" className="button-secondary ol-quote-cancel" onClick={onClose} disabled={saving}>Cancel</button><button type="button" aria-label="Save configuration" className="button-primary ol-quote-primary" onClick={() => void save()} disabled={saving || loading}><Save size={15} aria-hidden="true" />{saving ? "Saving…" : "Save"}</button></>}><div className="space-y-5">{loading ? <div className="py-12 text-center text-sm text-[var(--muted-foreground)]"><LoaderCircle className="mx-auto mb-2 animate-spin" size={22} aria-hidden="true" />Loading installment template…</div> : <><InfoBanner>{template?.banner || "No templates available. You can still configure installments manually by filling in the fields below."}</InfoBanner><FormGrid columns={3}><TextInput label="Annuity Period (years)" name="annuity_period_years" required type="number" min={1} max={plan?.policy_term_years} value={annuityPeriod} error={fieldError(errors, "annuity_period_years")} onChange={(event) => setAnnuityPeriod(event.target.value)} /><SmartSelect entity="payment-modes" label="Payment Mode" name="installment_payment_mode" required value={paymentMode} error={fieldError(errors, "payment_mode")} onChange={setPaymentMode} placeholder="Select payment mode" allowedValues={template?.available_payment_modes} hint={template?.available_payment_modes?.length ? `Valid payment methods for this product: ${template.available_payment_modes.join(", ")}. Premium frequency is configured in Step 2.` : "Payment methods are managed in OL Parameters → Drop Down Configuration; premium frequency is configured in Step 2."} /><ReadOnlyField label="Policy Term" required value={plan?.policy_term_years ? `${plan.policy_term_years} years` : "—"} hint="Policy term is inherited from plan configuration." /><ReadOnlyField label="Total Number of Installments" required value={rows.length} /></FormGrid><div className="grid gap-4 rounded-[10px] border bg-[var(--muted)]/25 p-4 md:grid-cols-2"><Toggle label="After Maturity Benefits" checked={afterMaturity} onChange={setAfterMaturity} hint="Member will get benefits after policy maturity" /><Toggle label="Before Maturity Benefits" checked={beforeMaturity} onChange={setBeforeMaturity} hint="Member will get benefits before policy maturity" /></div><InstallmentRateGrid rows={rows} onChange={setRows} error={fieldError(errors, "rate_rows") || (Math.abs(total - 100) >= 0.0001 ? "Installment rates must sum exactly to 100." : undefined)} /></>}</div></Modal>
}
...
function InstallmentsStep({ rows, loading, selectedPlan, template, templateLoading, saving, errors, onConfigure, onCloseModal, onSave, modalOpen }: { rows: InstallmentPlanRow[]; loading: boolean; selectedPlan: InstallmentPlanRow | null; template: InstallmentTemplate | null; templateLoading: boolean; saving: boolean; errors: ApiFieldErrors; onConfigure: (plan: InstallmentPlanRow) => void; onCloseModal: () => void; onSave: (payload: { annuity_period_years: number; payment_mode: string; after_maturity_benefits: boolean; before_maturity_benefits: boolean; rate_rows: InstallmentRateRow[] }) => Promise<boolean>; modalOpen: boolean }) {
  return <div className="surface-card overflow-hidden"><h2 className="sr-only">Installments</h2><div className="ol-quote-section-bar px-4 py-3"><div className="flex items-center gap-2"><CalendarClock size={17} aria-hidden="true" /><div><h3 className="text-base font-bold">Installment Configurations</h3><p className="mt-0.5 text-xs text-white/80">Configure installment payouts for whole life plans</p></div></div></div><div className="p-5">{loading ? <div className="py-12 text-center text-sm text-[var(--muted-foreground)]"><LoaderCircle className="mx-auto mb-2 animate-spin" size={22} aria-hidden="true" />Loading installment requirements…</div> : rows.length ? <div className="overflow-x-auto rounded-[10px] border"><table className="w-full min-w-[800px] text-left text-sm"><thead className="bg-[var(--muted)]/35 text-xs uppercase tracking-[0.08em] text-[var(--muted-foreground)]"><tr><th className="px-3 py-3">Plan/Sub-Product</th><th className="px-3 py-3">Policy Term (Years)</th><th className="px-3 py-3">Payment Mode</th><th className="px-3 py-3">No. of Installments</th><th className="px-3 py-3">Status</th><th className="px-3 py-3"><span className="sr-only">Action</span></th></tr></thead><tbody className="divide-y divide-[var(--border)]">{rows.map((row) => <tr key={row.plan_configuration_id}><td className="px-3 py-3"><p className="font-semibold">{row.plan_code}</p><p className="text-xs text-[var(--muted-foreground)]">{row.plan_name}</p></td><td className="px-3 py-3">{row.policy_term_years}</td><td className="px-3 py-3">{row.payment_mode || "Not Set"}</td><td className="px-3 py-3">{row.total_number_of_installments || "Not Set"}</td><td className="px-3 py-3"><span className={`rounded-full border px-2.5 py-1 text-xs font-bold ${row.status === "CONFIGURED" ? "badge-success" : "badge-neutral"}`}>{row.status === "CONFIGURED" ? "Configured" : "Ready to Configure"}</span></td><td className="px-3 py-3 text-right"><button type="button" className="button-primary ol-quote-primary !min-h-9 !px-3" disabled={!row.can_configure} onClick={() => onConfigure(row)}><Settings2 size={14} aria-hidden="true" />{row.status === "CONFIGURED" ? "Edit" : "Configure"}</button></td></tr>)}</tbody></table></div> : <div className="rounded-[10px] border border-dashed p-8 text-center text-sm text-[var(--muted-foreground)]">Select plans in Step 2 to configure installments.</div>}</div><ConfigureInstallmentModal open={modalOpen} plan={selectedPlan} template={template} loading={templateLoading} saving={saving} errors={errors} onClose={onCloseModal} onSave={onSave} /></div>
}
...
function RidersBenefitsStep({ state, optionsByPlan, errors, onSave }: { state: RiderState | null; optionsByPlan: Record<string, RiderOptions>; errors: ApiFieldErrors; onSave: (planConfigId: string, selections: RiderSelection[]) => Promise<boolean> }) {
  ...
  const rows = state?.plan_rows ?? []
  const selectedCount = rows.reduce((total, row) => total + (row.riders?.length ?? 0), 0)
  const edit = () => setConfigureOpen(true)
  return <div className="surface-card overflow-hidden"><h2 className="sr-only">Riders &amp; Benefits</h2><div className="space-y-5 p-5">{selectedCount > 0 ? <SelectedRidersTable rows={rows} onEdit={edit} /> : <section className="overflow-hidden rounded-[10px] border"><div className="flex flex-wrap items-center justify-between gap-3 border-b px-4 py-3"><div className="flex items-center gap-2"><Layers3 size={17} aria-hidden="true" className="text-[var(--muted-foreground)]" /><h3 className="font-bold">Selected Riders</h3></div><button type="button" className="button-primary ol-quote-primary !min-h-9 !px-3" onClick={edit}><Settings2 size={14} aria-hidden="true" />Configure Riders</button></div><InfoBanner className="m-3">No riders configured yet. Click "Configure Riders" to add riders to this quote.</InfoBanner></section>}{rows.length > 0 && <div className={selectedCount > 0 && !configureOpen ? "hidden" : "space-y-4"}>{rows.map((row) => <RiderPlanEditor key={row.plan_configuration_id} row={row} options={optionsByPlan[row.plan_configuration_id] ?? null} errors={errors} onSave={onSave} />)}</div>}{configureOpen && rows.length === 0 && <Modal open title="Configure Riders" description="Rider configurations are driven by the selected plan and OL Rider Setup parameters." onClose={() => setConfigureOpen(false)} footer={<button type="button" className="button-secondary" onClick={() => setConfigureOpen(false)}>Close</button>}><InfoBanner>No selected plan configuration is available for rider setup yet. Complete Plan &amp; Sub-Products first.</InfoBanner></Modal>}</div></div>
}
...
function FinancialDetailsStep({ quotation, summary, loading, calculating, onCalculate, onSaveInputs }: { quotation: Quotation; summary: FinancialDetails | null; loading: boolean; calculating: boolean; onCalculate: () => Promise<boolean>; onSaveInputs: (form: { currency: string; currency_rate: string; quote_validity_days: string }) => Promise<boolean> }) {
  const [currency, setCurrency] = useState(quotation.currency ?? "TZS")
  const [currencyRate, setCurrencyRate] = useState("1")
  const [quoteValidity, setQuoteValidity] = useState("30")
  useEffect(() => {
    setCurrency(quotation.currency ?? "TZS")
    if (quotation.expiry_date && quotation.quote_date) {
      const days = Math.round((new Date(quotation.expiry_date).getTime() - new Date(quotation.quote_date).getTime()) / 86400000)
      if (days > 0) setQuoteValidity(String(days))
    }
  }, [quotation.currency, quotation.expiry_date, quotation.quote_date])
  const saveInputs = async () => { await onSaveInputs({ currency, currency_rate: currencyRate, quote_validity_days: quoteValidity }) }
  const summaryCurrency = summary?.currency ?? currency ?? "TZS"
  ...
  return <div className="space-y-4">
    <section className="surface-card overflow-hidden"><h2 className="sr-only">Financial Details</h2><div className="border-b px-5 py-3"><p className="text-[11px] font-semibold text-[#a3a6b3]">Financial Details</p></div><div className="space-y-5 p-5"><FormGrid columns={3}><SmartSelect entity="currencies" label="Currency" name="quotation_currency" required value={currency} onChange={setCurrency} placeholder="Select currency" /><ReadOnlyField label="Currency Code" value={currency || "Auto-populated"} /><DecimalInput label="Currency Rate" name="quotation_currency_rate" required value={currencyRate} onChange={(event) => setCurrencyRate(event.target.value)} placeholder="Exchange rate" /></FormGrid><TextInput label="Quote Validity (days)" name="quote_validity_days" required type="number" min={1} value={quoteValidity} onChange={(event) => setQuoteValidity(event.target.value)} /><div className="flex justify-end"><button type="button" className="button-primary ol-quote-primary" onClick={() => void saveInputs()}><Save size={15} aria-hidden="true" />Save</button></div></div></section>
    <section className="surface-card overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-3 border-b px-5 py-4"><div><h3 className="font-bold">Premium Calculation</h3><p className="mt-1 text-sm text-[var(--muted-foreground)]">Premiums and projections are display-only outputs from the backend rating engine.</p></div><button type="button" className="button-primary ol-quote-primary" onClick={() => void onCalculate()} disabled={loading || calculating}>{calculating ? <><LoaderCircle size={15} className="animate-spin" aria-hidden="true" />Calculating…</> : <><RotateCcw size={15} aria-hidden="true" />{summary ? "Recalculate" : "Calculate"}</>}</button></div><div className="space-y-5 p-5">{summary?.recalculation_required && <InfoBanner>Quotation inputs changed after the last calculation. Recalculate before finalizing.</InfoBanner>}{loading ? <div className="py-10 text-center text-sm text-[var(--muted-foreground)]"><LoaderCircle className="mx-auto mb-2 animate-spin" size={22} aria-hidden="true" />Loading financial summary…</div> : summary ? <><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{cards.map((card) => <div key={card.label} className="rounded-[10px] border bg-[var(--muted)]/20 p-4"><p className="text-xs font-semibold text-[var(--muted-foreground)]">{card.label}</p><p className={`mt-2 text-lg font-extrabold ${card.label === "Total Premium" ? "text-[var(--primary)]" : ""}`}>{formatMoney(card.value, summaryCurrency)}</p><p className="mt-1 text-[10px] font-semibold uppercase tracking-[0.08em] text-[var(--muted-foreground)]">{summaryCurrency} · quotation frequency applied by engine</p></div>)}</div><div className="rounded-[12px] border border-[var(--primary)]/40 bg-[var(--primary)]/5 p-4"><p className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--muted-foreground)]">Estimated Maturity Value</p><p className="mt-1 text-2xl font-extrabold">{formatMoney(summary.estimated_maturity_value, summaryCurrency)}</p></div><div><h3 className="mb-3 font-bold">Policy-Year Projections</h3><div className="overflow-x-auto rounded-[10px] border"><table className="w-full min-w-[800px] text-left text-sm"><thead className="bg-[var(--muted)]/35 text-xs uppercase tracking-[0.08em] text-[var(--muted-foreground)]"><tr><th className="px-3 py-3">Policy Year</th><th className="px-3 py-3">Premiums Paid</th><th className="px-3 py-3">Bonuses</th><th className="px-3 py-3">Surrender Value</th><th className="px-3 py-3">Paid-Up Value</th></tr></thead><tbody className="divide-y divide-[var(--border)]">{(summary.projections ?? []).map((row) => <tr key={`${row.plan_configuration_id ?? "all"}-${row.policy_year}`}><td className="px-3 py-3 font-semibold">{row.policy_year}</td><td className="px-3 py-3">{formatMoney(row.premiums_paid, summaryCurrency)}</td><td className="px-3 py-3">{formatMoney(row.estimated_bonus, summaryCurrency)}</td><td className="px-3 py-3">{formatMoney(row.surrender_value, summaryCurrency)}</td><td className="px-3 py-3">{formatMoney(row.paid_up_value, summaryCurrency)}</td></tr>)}</tbody></table></div></div><div><h3 className="mb-3 font-bold">Installment Payout Schedule</h3><div className="overflow-x-auto rounded-[10px] border"><table className="w-full min-w-[800px] text-left text-sm"><thead className="bg-[var(--muted)]/35 text-xs uppercase tracking-[0.08em] text-[var(--muted-foreground)]"><tr><th className="px-3 py-3">Installment #</th><th className="px-3 py-3">Date</th><th className="px-3 py-3">Description</th><th className="px-3 py-3">Rate %</th><th className="px-3 py-3">Payout Amount</th></tr></thead><tbody className="divide-y divide-[var(--border)]">{(summary.installment_payouts ?? []).map((row) => <tr key={`${row.installment_configuration_id ?? "payout"}-${row.sequence}`}><td className="px-3 py-3 font-semibold">{row.sequence}</td><td className="px-3 py-3">{row.payout_date}</td><td className="px-3 py-3">{row.description ?? "—"}</td><td className="px-3 py-3">{row.rate_percent}</td><td className="px-3 py-3">{formatMoney(row.payout_amount, summaryCurrency)}</td></tr>)}</tbody></table></div></div></> : <div className="rounded-[10px] border border-dashed p-8 text-center text-sm text-[var(--muted-foreground)]">Calculate the quotation to view financial details.</div>}</div></section>
  </div>
}
...
export default function OLQuotationWizard() {
  ...
  const loadInvestmentFunds = useCallback(async () => {
    ...
    try {
      ...
      for (const row of payload.plan_rows ?? []) {
        nextAllocations[row.plan_configuration_id] = (row.allocations ?? []).map((allocation) => ({
          ...allocation,
          // The GET snapshot uses plan_configuration_id; POST requires plan_config_id.
          plan_config_id: allocation.plan_config_id || allocation.plan_configuration_id || row.plan_configuration_id,
        }))
      }
  ...
  const saveRiders = useCallback(async (planConfigId: string, selections: RiderSelection[]) => {
    ...
    const payloadSelections = Object.values(nextSelections).flat().map((selection) => {
      const options = riderOptions[String(selection.plan_config_id ?? planConfigId)]?.benefit_types ?? []
      const benefits = (selection.benefits ?? []).map((benefit) => ({
        ...benefit,
        beneficial_type_id: resolveBenefitTypeId(benefit.beneficial_type_id ?? benefit.benefit_type, options),
      }))
      return {
        rider_id: selection.rider_id,
        plan_config_id: selection.plan_config_id,
        rider_sum_assured: selection.rider_sum_assured,
        rider_term_years: selection.rider_term_years,
        beneficial_type_id: resolveBenefitTypeId(selection.beneficial_type_id ?? benefits[0]?.beneficial_type_id ?? selection.benefit_type, options),
        benefit_basis: selection.benefit_basis ?? "FIXED",
        benefit_value: selection.benefit_value,
        loading: selection.loading ?? 0,
        discount: selection.discount ?? 0,
        maximum_cap: selection.maximum_cap,
        benefits,
      }
    })
    setSaving(true)
  ...
  }, [loadRiders, quotationId, riderOptions, riderSelections, toast])
  ...
  const saveFinancialInputs = useCallback(async (form: { currency: string; currency_rate: string; quote_validity_days: string }) => {
    if (!quotationId) return false
    const validityDays = Number(form.quote_validity_days)
    if (!form.currency || !Number.isFinite(validityDays) || validityDays <= 0) {
      toast({ tone: "danger", title: "Financial details need attention", message: "Select a currency and enter a quote validity period greater than zero days." })
      return false
    }
    const expiry = new Date(`${personal.quote_date || today()}T00:00:00`)
    expiry.setDate(expiry.getDate() + validityDays)
    const expiryDate = expiry.toISOString().slice(0, 10)
    setSaving(true)
    try {
      const payload = await requestNormalized<Quotation>(`${QUOTATION_PREFIX}${quotationId}/`, { method: "PATCH", body: JSON.stringify({ currency: form.currency, expiry_date: expiryDate }) })
      setQuotation((current) => current ? { ...current, ...(payload ?? {}), currency: form.currency, expiry_date: expiryDate } : current)
      toast({ tone: "success", title: "Financial details saved" })
      return true
    } catch (error) {
      toast({ tone: "danger", title: "Financial details could not be saved", message: parseApiError(error).message })
      return false
    } finally { setSaving(false) }
  }, [personal.quote_date, quotationId, toast])
  ...
  const reviseQuotation = useCallback(async () => {
    if (!quotationId) return false
    setSaving(true)
    try {
      await requestNormalized(`${QUOTATION_PREFIX}${quotationId}/revise/`, { method: "POST", body: JSON.stringify({}) })
      await loadQuotation(quotationId)
      setFinancialSummary(null)
      setFinalizeErrors({})
      setInvalidSteps(new Set())
      setRevisionConfirmOpen(false)
      toast({ tone: "success", title: "Quotation revision created", message: "The quotation is now an editable draft." })
      return true
    } catch (error) {
      toast({ tone: "danger", title: "Quotation could not be revised", message: parseApiError(error).message })
      return false
    } finally { setSaving(false) }
  }, [loadQuotation, quotationId, toast])
  ...
  const saveFunds = useCallback(async () => {
    ...
    setFundErrors({})
    try {
      const allocations = Object.values(fundAllocations).flat().map((allocation) => ({
        plan_config_id: allocation.plan_config_id || allocation.plan_configuration_id || "",
  ...
  const quotationStatus = String(quotation?.status ?? "DRAFT").toUpperCase()
  const quotationIsEditable = quotationStatus === "DRAFT"
  ...
  return <div className="ol-quote-page space-y-4 p-3 sm:p-4 md:p-5">
    ...
    <Modal open={revisionConfirmOpen} title="Revise quotation" description="A new editable draft version will be created while preserving the finalized version for audit and comparison." onClose={() => setRevisionConfirmOpen(false)} size="sm" footer={<><button type="button" className="button-secondary" onClick={() => setRevisionConfirmOpen(false)} disabled={saving}>Cancel</button><button type="button" className="button-primary" onClick={() => void reviseQuotation()} disabled={saving}>{saving ? "Creating revision…" : "Revise and edit"}</button></>}><p className="text-sm leading-6 text-[var(--muted-foreground)]">The finalized quotation is read-only. Revising it creates a new editable version and requires recalculation before it can be finalized again.</p></Modal>
    <header className="ol-quote-hero relative overflow-visible"><div className="flex flex-wrap items-center justify-between gap-4 px-5 py-4 sm:px-6"><div className="flex min-w-0 items-center gap-3"><span className="ol-quote-hero-icon shrink-0"><FileText size={22} aria-hidden="true" /></span><div className="min-w-0"><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-white/75">Ordinary Life / Quotations</p><h1 className="mt-1 truncate text-xl font-bold tracking-tight sm:text-2xl">Create New Quote</h1></div></div><div className="ol-quote-summary"><span>{selectedPlanLabel}</span><button type="button" aria-expanded={detailsOpen} onClick={() => setDetailsOpen((current) => !current)}>Details <ChevronDown className="ml-1 inline" size={14} aria-hidden="true" /></button>{detailsOpen && <div className="absolute right-4 top-[calc(100%-0.25rem)] z-40 w-72 rounded-[10px] border border-white/20 bg-[var(--popover)] p-4 text-left text-sm text-[var(--foreground)] shadow-xl"><div className="mb-3 flex items-center justify-between border-b pb-3"><span className="font-semibold">Draft details</span><button type="button" aria-label="Close details" onClick={() => setDetailsOpen(false)}><X size={15} /></button></div><dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 text-xs"><dt className="text-[var(--muted-foreground)]">Quote number</dt><dd className="text-right font-semibold">{quotation.quote_number ?? "Pending"}</dd><dt className="text-[var(--muted-foreground)]">Status</dt><dd className="text-right font-semibold">{quotation.status ?? "DRAFT"}</dd><dt className="text-[var(--muted-foreground)]">Currency</dt><dd className="text-right font-semibold">{quotation.currency ?? "—"}</dd><dt className="text-[var(--muted-foreground)]">Expiry</dt><dd className="text-right font-semibold">{quotation.expiry_date ?? "—"}</dd></dl></div>}</div></div>{resumeNotice && <div className="flex items-center gap-2 border-t border-white/15 bg-black/10 px-5 py-2.5 text-xs font-semibold text-white/90"><RotateCcw size={14} aria-hidden="true" />Draft resumed from your last saved browser session.<button type="button" className="ml-auto underline" onClick={() => { localStorage.removeItem(LOCAL_DRAFT_KEY); setResumeNotice(false) }}>Dismiss</button></div>}</header>
    <WizardTabs activeStep={activeStep} completedSteps={completedSteps} invalidSteps={invalidSteps} onSelect={selectStep} />
    {!quotationIsEditable && <InfoBanner title={`Quotation ${quotationStatus === "FINALIZED" ? "finalized" : quotationStatus.toLowerCase()}`} className="items-start"><div className="flex flex-wrap items-center gap-3"><span>{quotationStatus === "FINALIZED" ? "This quotation is read-only. Create a revision to change any wizard step." : "This quotation cannot be edited in its current state."}</span>{quotationStatus === "FINALIZED" && <button type="button" className="button-primary !min-h-9 !px-3 !text-xs" onClick={() => setRevisionConfirmOpen(true)} disabled={saving}><Pencil size={14} aria-hidden="true" />Revise to edit</button>}</div></InfoBanner>}
    <div className="flex flex-col gap-4 lg:flex-row lg:items-start"><PlanSelectionPanel plans={plans} selectedPlanIds={selectedPlanIds} search={planSearch} loading={plansLoading} disabled={!quotationIsEditable} onSearch={setPlanSearch} onToggle={handlePlanToggle} onProductCreated={handleProductCreated} /><main className="min-w-0 flex-1"><fieldset disabled={!quotationIsEditable} className="m-0 min-w-0 border-0 p-0">
      ...
      {currentStep === "plans" && <div className="space-y-4"><div className="sr-only"><h2>Plan &amp; Sub-Products</h2><p>Step 2 of 7</p></div>{readablePlanErrors(planErrors.selection ?? {}).length > 0 && <div className="rounded-[10px] border border-[var(--destructive)]/35 bg-[var(--destructive)]/5 p-3 text-sm text-[var(--destructive)]" role="alert" aria-label="Plan selection validation errors"><p className="font-bold">Review plan selection</p><ul className="mt-2 list-disc space-y-1 pl-5">{readablePlanErrors(planErrors.selection ?? {}).map((message) => <li key={message}>{message}</li>)}</ul></div>}{!visibleConfigurations.length && <div className="surface-card rounded-[10px] p-8 text-center text-sm text-[var(--muted-foreground)]">Select one or more plans from the left panel, then continue to create configuration sections.</div>}{visibleConfigurations.map((config, index) => <PlanConfigurationSection key={config.id} index={index} config={config} card={cardByPlanId.get(configurationPlanId(config) ?? "")} options={planOptions} errors={planErrors[config.id] ?? {}} onChange={(field, value) => void patchConfiguration(config, field, value)} />)}</div>}
      ...
      {currentStep === "financial" && <div className="space-y-4"><FinancialDetailsStep quotation={quotation} summary={financialSummary} loading={financialLoading} calculating={calculating} onCalculate={calculateFinancials} onSaveInputs={saveFinancialInputs} /><FinancialPrerequisites paymentDetail={paymentDetail} underwritingDetail={underwritingDetail} paymentErrors={paymentErrors} underwritingErrors={underwritingErrors} saving={saving} onSavePayment={savePaymentDetails} onSaveUnderwriting={saveUnderwritingDetails} /><ReviewFinalize completedSteps={completedSteps} invalidSteps={invalidSteps} errors={finalizeErrors} onJump={(step) => setActiveStep(step)} onFinalize={finalizeQuotation} disabled={!quotationIsEditable || completedSteps.size < steps.length || !financialSummary || Boolean(financialSummary.recalculation_required) || !financialPrerequisitesComplete} /></div>}
    </fieldset></main></div>
    <footer className="ol-quote-footer surface-card flex flex-wrap items-center justify-between gap-3 px-5 py-4"><button type="button" className="button-secondary ol-quote-cancel" onClick={() => { localStorage.removeItem(LOCAL_DRAFT_KEY); sessionStorage.removeItem(ACTIVE_DRAFT_KEY); navigate("/ordinary-life/quotations") }}><X size={15} aria-hidden="true" />Cancel</button><div className="flex gap-2"><button type="button" className="button-secondary" disabled={activeStep === 0 || saving} onClick={() => setActiveStep((current) => Math.max(0, current - 1))}><ChevronLeft size={15} aria-hidden="true" />Previous</button><button type="button" className="button-primary ol-quote-primary" disabled={!quotationIsEditable || saving} onClick={() => void validateAndNavigate(activeStep + 1)}>{saving ? <LoaderCircle className="animate-spin" size={15} aria-hidden="true" /> : null}{activeStep === steps.length - 1 ? "Review & Finalize" : "Next"}<ChevronRight size={15} aria-hidden="true" /></button></div></footer>
  </div>
}
