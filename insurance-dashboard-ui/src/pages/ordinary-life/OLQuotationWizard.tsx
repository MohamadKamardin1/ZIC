  ...
  return <aside className="w-full shrink-0 lg:w-[310px] xl:w-[330px]" aria-label="Plan selection panel">
    <div className="surface-card ol-quote-plan-panel overflow-hidden">
      <header className="px-4 py-3">
        <div className="flex items-center justify-between gap-2"><div className="flex min-w-0 items-center gap-2"><PanelLeft size={17} aria-hidden="true" /><h2 className="truncate text-base font-bold">Plan Selection</h2></div>{canCreateProducts && <button type="button" className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md border border-white/35 text-white/90 transition hover:bg-white/15 focus-visible:ring-2 focus-visible:ring-white/70" aria-label="Add product" title="Add product" onClick={() => setQuickCreateOpen(true)}><Plus size={15} aria-hidden="true" /></button>}</div>
        <div className="relative mt-3"><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[var(--muted-foreground)]" size={16} aria-hidden="true" /><input aria-label="Search plans and sub-products" value={search} onChange={(event) => onSearch(event.target.value)} placeholder="Search plans and sub-products..." className="h-10 w-full rounded-[10px] border border-white/30 bg-white px-3 pl-9 text-sm text-slate-700 shadow-sm outline-none placeholder:text-slate-400 focus:border-white focus:ring-2 focus:ring-white/35" /></div>
      ...
      <div className="max-h-[calc(100vh-250px)] space-y-1 overflow-auto bg-white p-2.5">
        ...
        {!loading && plans.map((plan) => {
          ...
          return <button key={`${plan.product_version_id}-${plan.plan_id}`} type="button" aria-pressed={selected} onClick={() => onToggle(plan)} className={`w-full rounded-[8px] border p-3 text-left transition ${selected ? "border-[var(--ol-indigo)] bg-[#f8f8ff] shadow-sm" : "border-transparent bg-white hover:border-[#d8d9ee] hover:bg-[#fbfbfd]"}`}>
...
export default function OLQuotationWizard() {
  ...
  return <div className="ol-quote-page space-y-4 p-3 sm:p-4 md:p-5">
    ...
    <div className="flex flex-col gap-4 lg:flex-row lg:items-start"><PlanSelectionPanel plans={plans} selectedPlanIds={selectedPlanIds} search={planSearch} loading={plansLoading} onSearch={setPlanSearch} onToggle={handlePlanToggle} onProductCreated={handleProductCreated} /><main className="min-w-0 flex-1">
      ...
      {currentStep === "financial" && <div className="space-y-4"><FinancialDetailsStep quotation={quotation} summary={financialSummary} loading={financialLoading} calculating={calculating} onCalculate={calculateFinancials} onSaveInputs={saveFinancialInputs} /><FinancialPrerequisites paymentDetail={paymentDetail} underwritingDetail={underwritingDetail} paymentErrors={paymentErrors} underwritingErrors={underwritingErrors} saving={saving} onSavePayment={savePaymentDetails} onSaveUnderwriting={saveUnderwritingDetails} /><ReviewFinalize completedSteps={completedSteps} invalidSteps={invalidSteps} errors={finalizeErrors} onJump={(step) => setActiveStep(step)} onFinalize={finalizeQuotation} disabled={completedSteps.size < steps.length || !financialSummary || Boolean(financialSummary.recalculation_required) || !financialPrerequisitesComplete || quotation.status === "FINALIZED"} /></div>}
    </main></div>
    <footer className="ol-quote-footer surface-card flex flex-wrap items-center justify-between gap-3 px-5 py-4"><button type="button" className="button-secondary ol-quote-cancel" onClick={() => { localStorage.removeItem(LOCAL_DRAFT_KEY); sessionStorage.removeItem(ACTIVE_DRAFT_KEY); navigate("/ordinary-life/quotations") }}><X size={15} aria-hidden="true" />Cancel</button><div className="flex gap-2"><button type="button" className="button-secondary" disabled={activeStep === 0 || saving} onClick={() => setActiveStep((current) => Math.max(0, current - 1))}><ChevronLeft size={15} aria-hidden="true" />Previous</button><button type="button" className="button-primary ol-quote-primary" disabled={saving} onClick={() => void validateAndNavigate(activeStep + 1)}>{saving ? <LoaderCircle className="animate-spin" size={15} aria-hidden="true" /> : null}{activeStep === steps.length - 1 ? "Review & Finalize" : "Next"}<ChevronRight size={15} aria-hidden="true" /></button></div></footer>
  </div>
}
