...
const filters: Record<ScreenKey, FilterDefinition[]> = {
  ...
  "rate-tables": [{ key: "rider", label: "Rider", type: "select" }, { key: "product", label: "Product", type: "select" }, { key: "plan", label: "Plan", type: "select" }, { key: "rating_basis", label: "Rating basis", type: "select" }, { key: "is_active", label: "Status", type: "select" }],
  "rate-rows": [{ key: "table", label: "Rate table", type: "select" }, { key: "gender", label: "Gender", type: "select" }, { key: "smoker_status", label: "Smoker", type: "select" }, { key: "frequency", label: "Frequency", type: "select" }],
}
...
export default function OLRiderSetup() {
  ...
  const choiceFields = useMemo(() => screen === "riders" ? ["rider_category", "benefit_type", "calculation_basis"] : screen === "rate-tables" ? ["rating_basis", "rider", "product", "plan"] : ["gender", "smoker_status", "frequency", "rate_unit", "table"], [screen])
