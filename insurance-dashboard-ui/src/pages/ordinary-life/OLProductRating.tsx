...
type RelationOption = FilterOption
...
function Part2Editor({ tab, record, onChange, productOptions, planOptions }: { tab: Exclude<RatingTab, "premium" | "mortality" | "joint">; record: RatingRecord; onChange: (key: string, value: EditorValue) => void; productOptions: RelationOption[]; planOptions: RelationOption[] }) {
...
export default function OLProductRating() {
  ...
  const [productOptions, setProductOptions] = useState<RelationOption[]>([])
  const [planOptions, setPlanOptions] = useState<RelationOption[]>([])

  useEffect(() => {
    let active = true
    Promise.all([
      request<unknown>(`${API_PREFIX}/products/?page_size=200`),
      request<unknown>("/api/v1/ol/plans/search/?limit=200"),
    ]).then(([productPayload, planPayload]) => {
      if (!active) return
      const products = normalizeTableResponse<Record<string, unknown>>(productPayload).results
      const planRecord = planPayload && typeof planPayload === "object" ? planPayload as Record<string, unknown> : {}
      const plans = Array.isArray(planRecord.plans) ? planRecord.plans : normalizeTableResponse<Record<string, unknown>>(planPayload).results
      setProductOptions(products.flatMap((row) => {
        const value = row.id ?? row.code
        return value == null ? [] : [{ value: String(value), label: `${String(row.code ?? value)} — ${String(row.name ?? "")}`.trim() }]
      }))
      setPlanOptions((plans as Record<string, unknown>[]).flatMap((row) => {
        const value = row.id ?? row.plan_id ?? row.code
        return value == null ? [] : [{ value: String(value), label: `${String(row.code ?? value)} — ${String(row.name ?? "")}`.trim() }]
      }))
    }).catch(() => {
      if (active) { setProductOptions([]); setPlanOptions([]) }
    })
    return () => { active = false }
  }, [])
