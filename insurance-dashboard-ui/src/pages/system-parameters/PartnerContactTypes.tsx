...
import {
  fetchAllContactRequirements,
  createContactRequirement,
  updateContactRequirement,
  deleteContactRequirement,
  fetchPartnerTypes,
} from "../../lib/api"
import type { PartnerTypeContactRequirement, PartnerTypeRecord } from "../../lib/types"
...
export default function PartnerContactTypes() {
  const [items, setItems] = useState<PartnerTypeContactRequirement[]>([])
  ...
  const [editForm, setEditForm] = useState<Partial<PartnerTypeContactRequirement>>({})
  ...
  function startEdit(item: PartnerTypeContactRequirement) {
    setEditingId(item.id)
    setEditForm({ ...item })
  }

  async function handleAdd() {
    if (!form.partnerType || !form.contactType) return
    setSaving(true)
    try {
      const created = await createContactRequirement(form.partnerType, {
        contact_type: form.contactType,
        is_required: form.isRequired,
        multiple_allowed: form.multipleAllowed,
        display_order: form.displayOrder,
        is_active: form.isActive,
      } as unknown as Partial<PartnerTypeContactRequirement>)
  ...
  async function handleSave(id: string) {
    ...
    setSaving(true)
    try {
      const updated = await updateContactRequirement(ptId, id, editForm)
  ...
  async function handleDelete(id: string) {
    const ptId = items.find((i) => i.id === id)?.partnerType
    if (!ptId) return
    if (!window.confirm("Delete this contact requirement?")) return
    try {
      await deleteContactRequirement(ptId, id)
  ...
  const grouped = items.reduce<Record<string, PartnerTypeContactRequirement[]>>((acc, item) => {
  ...
  return (
    <div>
      <PageHeader title="Contact Config per Partner Type" description="Required contact types for each partner type">
        <button
          onClick={startAdd}
          disabled={adding}
      ...
      {adding && (
        <div className="mb-6 rounded-xl border border-border bg-card p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-foreground">New Contact Requirement</h3>
          ...
          <div className="flex justify-end gap-2">
            ...
            <button
              onClick={handleAdd}
              disabled={saving || !form.partnerType || !form.contactType}
              ...
              <div key={pt.id} className="rounded-xl border border-border bg-card overflow-hidden">
                ...
                {isExpanded && (
                  <div className="border-t border-border">
                    {contacts.length === 0 ? (
                      <p className="px-5 py-4 text-sm text-muted-foreground">No contact requirements defined for this partner type.</p>
                    ) : (
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="border-b border-border bg-muted/20 text-left text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                              <th className="px-4 py-2.5">Contact Type</th>
                              ...
                              return (
                                <tr key={c.id} className="border-b border-border/50 last:border-b-0">
                                  {isEditing ? (
                                    <>
                                      <td className="px-4 py-2">
                                        <input
                                          ...
                                          onChange={(e) => setEditForm((ef) => ({ ...ef, contactType: e.target.value }))}
                                    ...
                                    <>
                                      <td className="px-4 py-2.5 font-medium text-foreground">{c.contactType}</td>
                                      <td className="px-4 py-2.5">
                                        <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium ${
                                          c.isRequired ? "bg-[var(--color-bg-success-soft)] text-[var(--color-text-success-soft)]" : "bg-[var(--color-bg-warning-soft)] text-[var(--color-text-warning-soft)]"
                                        }`}>
                                      ...
                                      <td className="px-4 py-2.5">
                                        <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium ${
                                          c.multipleAllowed ? "bg-[var(--color-bg-success-soft)] text-[var(--color-text-success-soft)]" : "bg-[var(--color-bg-warning-soft)] text-[var(--color-text-warning-soft)]"
                                        }`}>
                                          {c.multipleAllowed ? "Yes" : "No"}
                                        </span>
                                      </td>
                                      <td className="px-4 py-2.5 text-xs text-muted-foreground">{c.displayOrder}</td>
                                      <td className="px-4 py-2.5">
                                        <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium ${
                                          c.isActive ? "bg-[var(--color-bg-success-soft)] text-[var(--color-text-success-soft)]" : "bg-[var(--color-bg-destructive-soft)] text-[var(--color-text-destructive-soft)]"
                                        }`}>
                                      ...
                                      <td className="px-4 py-2.5 text-right">
                                        <div className="flex items-center justify-end gap-1">
                                          <button onClick={() => startEdit(c)} className="rounded p-1 text-muted-foreground hover:text-foreground hover:bg-accent" title="Edit">
                                            <Pencil className="h-3.5 w-3.5" />
                                          </button>
                                          <button onClick={() => handleDelete(c.id)} className="rounded p-1 text-muted-foreground hover:text-destructive hover:bg-destructive/10" title="Delete">
