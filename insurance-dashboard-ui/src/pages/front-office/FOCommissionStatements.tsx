...
export default function FOCommissionStatements() {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const data = await foCore.listCommissionStatements();
      setItems(data);
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        await foCore.updateCommissionStatement(editingItem.id, formData);
      } else {
        await foCore.createCommissionStatement(formData);
      }
      closeModal();
      fetchItems();
    } catch (err: any) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await foCore.deleteCommissionStatement(id);
      fetchItems();
    } catch (err: any) {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="mb-6 flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage Commission Statements</p>
        </div>
        <button
          onClick={() => openModal()}
        ...
        >
          <Plus className="h-4 w-4" /> Add Commission Statements
        </button>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-foreground">
            <thead className="bg-secondary/30 text-muted-foreground border-b border-border">
              <tr>
                <th className="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Agent ID</th>
                <th className="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Period Start</th>
                <th className="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Period End</th>
              ...
              {loading ? (
                ...
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-secondary/20 transition-colors">
                    <td className="px-6 py-4">{item.agent_id}</td>
                    <td className="px-6 py-4">{item.period_start}</td>
                    <td className="px-6 py-4">{item.period_end}</td>
          ...
          <div className="bg-card border border-border rounded-2xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95">
            <div className="px-6 py-4 border-b border-border flex justify-between items-center bg-card">
              <h3 className="text-lg font-bold text-foreground">
                {editingItem ? 'Edit' : 'Add'} Commission Statements
              </h3>
