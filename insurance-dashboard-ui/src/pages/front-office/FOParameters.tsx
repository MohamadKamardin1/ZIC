...
import { Settings, Plus, Search, Edit2, Trash2, X, Loader2, AlertCircle } from "lucide-react";
import { foCore } from "../../lib/fo-api";

export default function FOParameters() {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const data = await foCore.listParameters();
      setItems(data);
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        await foCore.updateParameter(editingItem.id, formData);
      } else {
        await foCore.createParameter(formData);
      }
      closeModal();
      fetchItems();
    } catch (err: any) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await foCore.deleteParameter(id);
      fetchItems();
    } catch (err: any) {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="mb-6 flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage Front Office Parameters</p>
        </div>
        <button
          onClick={() => openModal()}
        ...
        >
          <Plus className="h-4 w-4" /> Add Front Office Parameters
        </button>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-foreground">
            <thead className="bg-secondary/30 text-muted-foreground border-b border-border">
              <tr>
                <th className="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Key</th>
                <th className="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Value</th>
          ...
          <div className="bg-card border border-border rounded-2xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95">
            <div className="px-6 py-4 border-b border-border flex justify-between items-center bg-card">
              <h3 className="text-lg font-bold text-foreground">
                {editingItem ? 'Edit' : 'Add'} Front Office Parameters
              </h3>
