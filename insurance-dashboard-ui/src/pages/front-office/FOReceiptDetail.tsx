...
import { ReceiptDocumentsPanel, ReceiptPrintPreviewModal } from "../../components/receipts/ReceiptPrintPreview"
...
import { openAuthenticatedDocument } from "../../lib/documentClient"
...
type DetailTab = typeof DETAIL_TABS[number]["id"] | "legacy-documents"
...
export default function FOReceiptDetail() {
  ...
  const [documentToPreview, setDocumentToPreview] = useState<ReceiptDocument | null>(null)
  ...
  const downloadReceiptDocument = async (document: ReceiptDocument) => {
    const url = document.signed_download_url ?? document.download_url
    if (!url) {
      setActionMessage("The document service did not return a secure PDF URL. Generate the receipt again or contact System Administration.")
      return
    }
    try {
      await openAuthenticatedDocument(url, { kind: "pdf", mode: "download", filename: `${receipt?.receipt_number ?? "receipt"}.pdf` })
    } catch (error) {
      setActionMessage(error instanceof Error ? error.message : "The receipt PDF could not be downloaded.")
    }
  }
...
>Reverse allocation<ChevronRight size={13} aria-hidden="true" /></button>}</td></tr>)}</tbody></table></div>}</section>}
    ...
    <ReceiptPrintPreviewModal open={printPreviewOpen} receipt={receipt} document={documentToPreview} onClose={() => { setPrintPreviewOpen(false); setDocumentToPreview(null) }} onGenerated={() => { void queryClient.invalidateQueries({ queryKey: ["receipts", "detail", id, "documents"] }) }} />
  </div>
}
