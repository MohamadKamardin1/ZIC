...
import { glQuotations, glSetup } from "../../lib/gl-api"
...
export default function GLQuotations() {
  ...
  const loadQuotations = useCallback(async () => {
    ...
    try {
      ...
      const res = await glQuotations.list(params)
  ...
  useEffect(() => {
    Promise.all([
      glSetup.listProducts().then((r: any) => setProducts(r?.results ?? r?.data ?? r ?? [])),
      glSetup.listSchemeTypes().then((r: any) => setSchemeTypes(r?.results ?? r?.data ?? r ?? [])),
    ]).catch(() => {})
  }, [])
  ...
  async function viewDetail(id: string) {
    ...
    try {
      const detail = await glQuotations.get(id)
  ...
  async function handleCreate() {
    setSaving(true)
    try {
      await glQuotations.create(formData)
  ...
  async function handleAction(action: string, id: string) {
    ...
    try {
      if (action === "approve") await glQuotations.approve(id)
      else if (action === "decline") await glQuotations.decline(id, "Declined by user")
      else if (action === "convert") await glQuotations.convertToScheme(id, {})
      await viewDetail(id)
      await loadQuotations()
    } catch (err: any) {
      alert(err.message)
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-6 flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage Group Life insurance quotations</p>
        </div>
        <button
