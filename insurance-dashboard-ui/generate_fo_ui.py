import os

components = [
    {
        "name": "FOReceipts",
        "title": "Receipts",
        "icon": "FileText",
        "endpoint": "foCore.listReceipts",
        "create_endpoint": "foCore.createReceipt",
        "update_endpoint": "foCore.updateReceipt",
        "delete_endpoint": "foCore.deleteReceipt",
        "fields": [
            {"key": "receipt_number", "label": "Receipt No.", "type": "text"},
            {"key": "amount", "label": "Amount", "type": "number"},
            {"key": "payment_method", "label": "Payment Method", "type": "text"},
            {"key": "payment_date", "label": "Payment Date", "type": "date"},
            {"key": "reference", "label": "Reference", "type": "text"},
            {"key": "status", "label": "Status", "type": "select", "options": ["COMPLETED", "FAILED", "PENDING"]},
        ]
    },
    {
        "name": "FOCommissions",
        "title": "Commissions",
        "icon": "FileText",
        "endpoint": "foCore.listCommissions",
        "create_endpoint": "foCore.createCommission",
        "update_endpoint": "foCore.updateCommission",
        "delete_endpoint": "foCore.deleteCommission",
        "fields": [
            {"key": "agent_id", "label": "Agent ID", "type": "text"},
            {"key": "policy_reference", "label": "Policy Ref", "type": "text"},
            {"key": "amount", "label": "Amount", "type": "number"},
            {"key": "status", "label": "Status", "type": "select", "options": ["PENDING", "PAID", "CANCELLED"]},
        ]
    },
    {
        "name": "FOCommissionStatements",
        "title": "Commission Statements",
        "icon": "FileText",
        "endpoint": "foCore.listCommissionStatements",
        "create_endpoint": "foCore.createCommissionStatement",
        "update_endpoint": "foCore.updateCommissionStatement",
        "delete_endpoint": "foCore.deleteCommissionStatement",
        "fields": [
            {"key": "agent_id", "label": "Agent ID", "type": "text"},
            {"key": "period_start", "label": "Period Start", "type": "date"},
            {"key": "period_end", "label": "Period End", "type": "date"},
            {"key": "total_amount", "label": "Total Amount", "type": "number"},
            {"key": "status", "label": "Status", "type": "select", "options": ["DRAFT", "ISSUED", "PAID"]},
        ]
    },
    {
        "name": "FORequisitions",
        "title": "Requisitions",
        "icon": "FileText",
        "endpoint": "foCore.listRequisitions",
        "create_endpoint": "foCore.createRequisition",
        "update_endpoint": "foCore.updateRequisition",
        "delete_endpoint": "foCore.deleteRequisition",
        "fields": [
            {"key": "requisition_number", "label": "Req. No.", "type": "text"},
            {"key": "department", "label": "Department", "type": "text"},
            {"key": "amount", "label": "Amount", "type": "number"},
            {"key": "reason", "label": "Reason", "type": "text"},
            {"key": "status", "label": "Status", "type": "select", "options": ["PENDING", "APPROVED", "DECLINED"]},
        ]
    },
    {
        "name": "FOPayments",
        "title": "Payments",
        "icon": "FileText",
        "endpoint": "foCore.listPayments",
        "create_endpoint": "foCore.createPayment",
        "update_endpoint": "foCore.updatePayment",
        "delete_endpoint": "foCore.deletePayment",
        "fields": [
            {"key": "payment_number", "label": "Payment No.", "type": "text"},
            {"key": "amount", "label": "Amount", "type": "number"},
            {"key": "payment_method", "label": "Method", "type": "text"},
            {"key": "payment_date", "label": "Date", "type": "date"},
            {"key": "recipient", "label": "Recipient", "type": "text"},
            {"key": "status", "label": "Status", "type": "select", "options": ["PENDING", "COMPLETED", "FAILED"]},
        ]
    },
    {
        "name": "FOParameters",
        "title": "Front Office Parameters",
        "icon": "Settings",
        "endpoint": "foCore.listParameters",
        "create_endpoint": "foCore.createParameter",
        "update_endpoint": "foCore.updateParameter",
        "delete_endpoint": "foCore.deleteParameter",
        "fields": [
            {"key": "key", "label": "Key", "type": "text"},
            {"key": "value", "label": "Value", "type": "text"},
            {"key": "description", "label": "Description", "type": "text"},
        ]
    }
]

template = """import {{ useState, useEffect }} from "react";
import {{ {icon}, Plus, Search, Edit2, Trash2, X, Loader2, AlertCircle }} from "lucide-react";
import {{ foCore }} from "../../lib/fo-api";

export default function {name}() {{
  const [items, setItems] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [editingItem, setEditingItem] = useState<any>(null);
  const [formData, setFormData] = useState<any>({{}});

  const fetchItems = async () => {{
    try {{
      setLoading(true);
      const data = await {endpoint}();
      setItems(data);
    }} catch (err: any) {{
      setError(err.message || "Failed to load data.");
    }} finally {{
      setLoading(false);
    }}
  }};

  useEffect(() => {{
    fetchItems();
  }}, []);

  const openModal = (item?: any) => {{
    setEditingItem(item || null);
    setFormData(item || {{}});
    setIsModalOpen(true);
  }};

  const closeModal = () => {{
    setIsModalOpen(false);
    setFormData({{}});
    setEditingItem(null);
  }};

  const handleSubmit = async (e: React.FormEvent) => {{
    e.preventDefault();
    try {{
      setIsSubmitting(true);
      if (editingItem) {{
        await {update_endpoint}(editingItem.id, formData);
      }} else {{
        await {create_endpoint}(formData);
      }}
      closeModal();
      fetchItems();
    }} catch (err: any) {{
      alert(err.message || "Failed to save record.");
    }} finally {{
      setIsSubmitting(false);
    }}
  }};

  const handleDelete = async (id: string) => {{
    if (!confirm("Are you sure you want to delete this record?")) return;
    try {{
      await {delete_endpoint}(id);
      fetchItems();
    }} catch (err: any) {{
      alert(err.message || "Failed to delete record.");
    }}
  }};

  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="mb-6 flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-3">
            <div className="p-2 rounded-xl" style={{{{ background: "linear-gradient(135deg, #3b82f6, #6366f1)" }}}}>
              <{icon} className="h-6 w-6 text-white" />
            </div>
            {title}
          </h1>
          <p className="text-muted-foreground mt-1">Manage {title}</p>
        </div>
        <button
          onClick={{() => openModal()}}
          className="flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-medium text-white shadow-lg transition hover:opacity-90"
          style={{{{ background: "linear-gradient(135deg, #3b82f6, #6366f1)" }}}}
        >
          <Plus className="h-4 w-4" /> Add {title}
        </button>
      </div>

      {{error && (
        <div className="p-4 bg-destructive/10 border border-destructive/20 rounded-xl flex items-center text-destructive">
          <AlertCircle className="w-5 h-5 mr-3" />
          {{error}}
        </div>
      )}}

      <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
        <div className="p-4 border-b border-border flex justify-between items-center bg-secondary/30">
          <div className="relative w-64 max-w-sm">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
            <input
              type="text"
              placeholder="Search..."
              className="h-10 w-full rounded-xl border border-border bg-card pl-10 pr-4 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/40"
            />
          </div>
        </div>

        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-foreground">
            <thead className="bg-secondary/30 text-muted-foreground border-b border-border">
              <tr>
                {table_headers}
                <th className="px-6 py-4 font-semibold uppercase tracking-wider text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {{loading ? (
                <tr>
                  <td colSpan={{10}} className="px-6 py-8 text-center text-muted-foreground">
                    <Loader2 className="w-6 h-6 animate-spin mx-auto mb-2 text-primary" />
                    Loading...
                  </td>
                </tr>
              ) : items.length === 0 ? (
                <tr>
                  <td colSpan={{10}} className="px-6 py-8 text-center text-muted-foreground">
                    No records found.
                  </td>
                </tr>
              ) : (
                items.map((item) => (
                  <tr key={{item.id}} className="hover:bg-secondary/20 transition-colors">
                    {table_cells}
                    <td className="px-6 py-4 text-right">
                      <button onClick={{() => openModal(item)}} className="text-muted-foreground hover:text-primary p-1.5 rounded-lg hover:bg-secondary mr-2 transition-colors">
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button onClick={{() => handleDelete(item.id)}} className="text-muted-foreground hover:text-destructive p-1.5 rounded-lg hover:bg-secondary transition-colors">
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </td>
                  </tr>
                ))
              )}}
            </tbody>
          </table>
        </div>
      </div>

      {{/* Modal */}}
      {{isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-2xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95">
            <div className="px-6 py-4 border-b border-border flex justify-between items-center bg-card">
              <h3 className="text-lg font-bold text-foreground">
                {{editingItem ? 'Edit' : 'Add'}} {title}
              </h3>
              <button onClick={{closeModal}} className="text-muted-foreground hover:text-foreground p-2 rounded-lg hover:bg-secondary transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            
            <div className="p-6 overflow-y-auto">
              <form id="record-form" onSubmit={{handleSubmit}} className="space-y-4">
                {form_inputs}
              </form>
            </div>

            <div className="px-6 py-4 border-t border-border bg-secondary/30 flex justify-end space-x-3">
              <button
                type="button"
                onClick={{closeModal}}
                className="px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors"
              >
                Cancel
              </button>
              <button
                type="submit"
                form="record-form"
                disabled={{isSubmitting}}
                className="bg-primary hover:bg-primary/90 text-primary-foreground px-6 py-2 rounded-xl text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
              >
                {{isSubmitting && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}}
                Save
              </button>
            </div>
          </div>
        </div>
      )}}
    </div>
  );
}}
"""

os.makedirs("src/pages/front-office", exist_ok=True)

for comp in components:
    table_headers = "\n                ".join([f'<th className="px-6 py-4 font-semibold uppercase tracking-wider text-xs">{f["label"]}</th>' for f in comp["fields"]])
    table_cells = "\n                    ".join([f'<td className="px-6 py-4">{{item.{f["key"]}}}</td>' for f in comp["fields"]])
    
    form_inputs = ""
    for f in comp["fields"]:
        if f["type"] == "select":
            opts = "\n                    ".join([f'<option value="{opt}">{opt}</option>' for opt in f["options"]])
            form_inputs += f"""
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">{f['label']}</label>
                  <select
                    required
                    value={{formData.{f['key']} || ''}}
                    onChange={{e => setFormData({{ ...formData, {f['key']}: e.target.value }})}}
                    className="w-full h-10 bg-card border border-border rounded-xl px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/40"
                  >
                    <option value="">Select...</option>
                    {opts}
                  </select>
                </div>"""
        else:
            input_type = f["type"]
            form_inputs += f"""
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">{f['label']}</label>
                  <input
                    type="{input_type}"
                    required
                    value={{formData.{f['key']} || ''}}
                    onChange={{e => setFormData({{ ...formData, {f['key']}: { 'Number(e.target.value)' if input_type=='number' else 'e.target.value' } }})}}
                    className="w-full h-10 bg-card border border-border rounded-xl px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/40"
                  />
                </div>"""

    # Filter out duplicate imports just in case
    icon_str = comp["icon"]
    if icon_str in ["Plus", "Search", "Edit2", "Trash2", "X", "Loader2", "AlertCircle"]:
        icon_import = ""
    else:
        icon_import = f"{icon_str},"
        
    content = template.format(
        name=comp["name"],
        title=comp["title"],
        icon=comp["icon"],
        endpoint=comp["endpoint"],
        create_endpoint=comp["create_endpoint"],
        update_endpoint=comp["update_endpoint"],
        delete_endpoint=comp["delete_endpoint"],
        table_headers=table_headers,
        table_cells=table_cells,
        form_inputs=form_inputs
    )
    
    # fix the imports dynamically to avoid duplicates like previously
    content = content.replace(f"import {{ {icon_str}, Plus, Search, Edit2, Trash2, X, Loader2, AlertCircle }} from", 
                              f"import {{ {icon_import} Plus, Search, Edit2, Trash2, X, Loader2, AlertCircle }} from")

    with open(f"src/pages/front-office/{comp['name']}.tsx", "w") as f:
        f.write(content)
