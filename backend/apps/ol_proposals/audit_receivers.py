...
def _capture_before_state(sender, instance, **kwargs):
    ...
    if previous is not None:
        instance._ol_proposals_before_state = AuditService.snapshot(previous)


def _audit_save(sender, instance, created, **kwargs):
    if _is_suppressed():
        return
    before = getattr(instance, "_ol_proposals_before_state", None)
    actor = _actor_for(instance)
    channel = _channel_for(instance)
    label = str(instance._meta.verbose_name or "").replace("_", " ")
    if created:
        AuditService.log_create(instance, actor=actor, reason=f"{label} created.", source_channel=channel)
    ...
    if hasattr(instance, "_ol_proposals_before_state"):
        delattr(instance, "_ol_proposals_before_state")
...
for _model in AUDITED_MODELS:
    pre_save.connect(
        ...
        sender=_model,
        dispatch_uid=f"ol_proposals_{_model.__name__.lower()}_before_save",
    )
    post_save.connect(
        _audit_save,
        sender=_model,
        dispatch_uid=f"ol_proposals_{_model.__name__.lower()}_after_save",
    )
