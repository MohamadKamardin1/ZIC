...
CLAIM_ERROR_REGISTRY = {
    ...
    "CLAIM_SETTLEMENT_NOT_READY": {
        "message": "This claim is not ready to be settled in its current status.",
        "status_code": 422,
        "resolution_steps": [
    ...
    "CLAIM_SETTLEMENT_REQUISITION_REQUIRED": {
        "message": "A payment requisition is required before this claim can be settled.",
        "status_code": 422,
        "resolution_steps": [
            "Raise a payment requisition from the assessed claim.",
            "Confirm that the Front Office payment request is linked, then retry settlement.",
        ],
    },
    "CLAIM_SETTLEMENT_APPROVAL_REQUIRED": {
        "message": "The claim payment approval is still pending.",
        "status_code": 422,
        "resolution_steps": [
            "Open the linked approval request and complete the finance approval decision.",
            "Retry settlement only after the claim status changes to Approved.",
        ],
    },
    "CLAIM_SETTLEMENT_PAYMENT_NOT_CONFIRMED": {
        "message": "Settlement requires confirmation from the Front Office payment process.",
        "status_code": 422,
        "resolution_steps": [
            "Wait for Front Office to confirm, mark paid, or complete the payment.",
            "Submit the confirmed payment status and reference when retrying settlement.",
        ],
    },
    "CLAIM_SETTLEMENT_PAYMENT_REFERENCE_REQUIRED": {
        "message": "A Front Office payment reference is required before settlement.",
        "status_code": 400,
        "resolution_steps": [
