...
Configuration stores the annuity period, parameter-backed payment method, maturity-benefit toggles, and ordered rate rows. The documented computation assumption is `total_number_of_installments = len(rate_rows)`; the persisted configuration therefore reports the number of saved allocation rows rather than deriving a payment count from premium frequency alone. Rate percentages must sum exactly to `100`, each sequence must be unique, and the annuity period must be positive and no greater than the inherited policy term. **Premium frequency and payment mode are different settings:** premium frequency (`ANNUAL`, `SEMI_ANNUAL`, `QUARTERLY`, or `MONTHLY`) is resolved from the selected product version and is used for rating and installment-template lookup; payment mode (`CASH`, `BANK_TRANSFER`, or `MOBILE_MONEY`) is resolved from the active `OL_PAYMENT_MODE_CHOICES` catalog and is stored on the installment configuration. A product version may optionally restrict valid payment methods with `servicing_rules.installment_payment_modes`; unknown or inactive catalog values are rejected. If the selected plan or product rules require installment benefits, at least one maturity toggle must be selected. The inherited policy term and installment amount are server-controlled; a positive estimated maturity value must exist before a new configuration can be saved. Successful saves mark wizard completion under the key `4_installments`, increment the quotation version, write an audit/outbox event, and transition the plan row from `READY_TO_CONFIGURE` to `CONFIGURED`.

### Where administrators configure installment inputs

1. **Product and premium frequency:** `Ordinary Life Parameters → Product Setup → OL Product`. Configure eligibility, term limits, currency, capabilities, and **Premium frequencies**. This controls the `Payment Frequency` field in Step 2 and the premium/rate-template lookup.
2. **Payment methods:** `Ordinary Life Parameters → Drop Down Configuration → Payment Modes`. Maintain active values such as Cash, Bank transfer, and Mobile money. These populate the Step 4 **Payment Mode** selector and are the values accepted by the installment API.
3. **Quotation-specific installment attributes:** `Ordinary Life Quotations → Create New Quote → Installments → Configure`. Configure annuity period, maturity-benefit timing, and payout-rate rows for the selected plan. The policy term is inherited from Step 2 and cannot be edited in this modal.
4. **Product-specific payment-method restriction:** When a product version needs a restricted method list, open the Django admin **Ordinary Life → Product Versions**, edit the effective version, and maintain `servicing_rules.installment_payment_modes` in the **Installment payment methods** section. If no restriction is supplied, all active payment modes from Drop Down Configuration are valid. The API returns the resulting allowed values in `available_payment_modes`, and the frontend filters the selector to those values.

The implementation deliberately never uses product premium frequencies as installment payment modes. For example, selecting `Bank transfer` is valid for a product whose premium frequency is `Annual`; the two values govern different business concepts.
