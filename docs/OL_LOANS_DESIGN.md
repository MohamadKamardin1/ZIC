...
## Prompt 12 release seed and verification rules

The final release command is `python3 manage.py seed_ol_loan_release --json`. It is additive and idempotent: it runs the complete Zanzibar OL baseline unless `--skip-baseline` is supplied, then creates exactly ten deterministic loans under the `OL-RELEASE-` namespace. The ten scenarios are standard Active, Partially Repaid, Settled, Defaulted, Offset on Surrender, Offset on Claim, Multi-currency Repayment, Rejected Request, Pending Approval, and an Import-source Active loan. The import scenario uses a manual batch path because the current OL Loans context does not expose a CSV import contract.

The command uses the request, approval, rejection, disbursement, repayment, default detection, and offset services. It also creates or reuses policy, proposal, quotation, partner, payment-mode, bank-account, and effective loan-parameter records needed to exercise the complete graph. The seed uses explicit dates of 2026-01-15 for release and 2026-08-27 for verification so financial evidence is reproducible. A successful JSON result reports `loan_count=10`, all scenario states and balances, repayment/offset counts, audit counts, and five failure proofs.

The failure proofs must catch an ineligible policy, a request above the cash-value limit, an overpayment, and an offset against an already settled loan. The duplicate disbursement proof must return the existing immutable disbursement with `created=false` and no second release. Each negative proof includes the stable error code, human-readable message, safe details, field guidance, and resolution steps. Failed operations run inside savepoint transactions, so no unintended financial rows are committed.

Prompt 12 release acceptance requires the targeted release tests, the complete affected Django test matrix, migration drift check, Django system check, Python compile check, `verify_loan_audit --json`, and `git diff --check`. The saved prompt series is checked only after these checks pass and the implementation commit is pushed to the non-force `sultan` delivery branch.
