...
## Prompt 8/10 — Payment Readiness, First Premium, Policy Conversion, Cancel and Print

You are a senior frontend engineer. Continue the ZIC Proposals UI. Execute ONLY Prompt 8.

MANDATORY RULES:
- BR-03 must be visible and teachable: conversion to policy is impossible until the first premium is fully posted.
- Money actions are confirmed: cancellation requires a ReasonField and a danger-styled confirmation stage.
- All failures surface through ErrorCoach; names and labels never raw codes.

SCOPE:
1. Mark Payment Ready: on 409 PROPOSAL_NOT_PAYMENT_READY render the failed checklist items in destructive styling with resolution steps and deep links into the fixing screens.
2. FirstPremiumCard upgrade: commitment number/status, amount due/paid/balance, allocations mini-table with receipt references; receipt-record hint while the premium is unposted.
3. Convert to Policy modal: live first-premium status fetch, BR-03 summary block (commitment, due/paid/balance), blocked state via ErrorCoach with PROPOSAL_FIRST_PREMIUM_NOT_POSTED plus manual steps; success view links the issued policy number; lifecycle banner after conversion.
4. Cancel modal: two-stage flow — mandatory reason (min length) then ConfirmModal danger stage restating the reason before submission; cancelled lifecycle banner shows the recorded reason.
5. Print preview modal: generates a durable print document (POST /print/), previews the PDF in an iframe with template/version/source metadata and a download link.
6. Generated Documents tab: table of durable printouts (document type, template + version, source version, generated by/at) fed by GET {id}/generated-documents/.

TESTS:
- Failed checklist rendering with resolution steps and deep links on 409 refusal
- First premium partial vs posted states
- Convert blocked before premium posts with correct coach; convert success shows policy link
- Cancel requires reason then danger confirm before submission; cancelled banner
- Print preview metadata/frame/download and generated documents list

GIT:
- commit: "feat(web): payment ready first premium conversion and print UI"
- push; tick checkbox
