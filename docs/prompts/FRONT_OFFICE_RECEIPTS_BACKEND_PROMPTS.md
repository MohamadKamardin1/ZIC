...
- [ ] Prompt 8 — Implement Receipt Printout & Document Integration
...
## Prompt 8/12 — Implement Receipt Printout & Document Integration

```text
You are a senior Django document engineer. Continue the Front Office Receipts backend. Execute ONLY Prompt 8.

MANDATORY RULES:
- Use the existing unified print/PDF engine.
- Generated receipt documents must retain source transaction and template version.
- Commit and push; tick Prompt 8 after completion.

OBJECTIVE:
Implement receipt printout generation.

SCOPE:
1. Register document template type: RECEIPT.
2. Receipt PDF must include:
   - company logo and company details
   - receipt number
   - receipt date
   - branch
   - payer name and identity/partner number
   - payment mode
   - payment reference
   - currency
   - amount in figures and words
   - allocated commitments table
   ...
   - signature lines
   - reversal/cancel watermark if applicable
   - template version footer
3. API:
   - POST /api/v1/front-office/receipts/{id}/print/
   - GET /api/v1/front-office/receipts/{id}/documents/
4. Print rules:
   - DRAFT can print preview only if permission allows
   - POSTED/PARTIALLY_ALLOCATED/FULLY_ALLOCATED can print official receipt
   - REVERSED prints with reversal watermark
   - CANCELLED prints with cancelled watermark
5. Use authenticated print pipeline and signed ticket.
6. Audit print generation and download.

TESTS:
- receipt PDF generated
- pypdf extraction verifies required blocks
- watermark tests
- permission denial
- document instance linked to receipt and template version

GIT:
- commit: "feat(receipts): implement receipt printout and document integration"
- push; tick Prompt 8 checkbox
