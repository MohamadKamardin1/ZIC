...
## Claims register, detail, KPI, and export APIs

Prompt 10 completes the table-first Claims register. `GET /api/v1/ol/claims/` supports search over claim number, policy number, claimant or policyholder name; filters for status, claim type, product, branch/location, claim-date range, and fraud flag; server-side pagination; and an allowlisted sort field. The branch filter follows the issued policy lineage to the source quotation location and its branch code or name because the current issued policy snapshot does not duplicate a branch foreign key. Every table row contains claim number, policy number, policyholder name and display, product display, claim type, dates, amount, currency, status display, fraud flag, and actions; no human-readable label contains a UUID.

`GET /api/v1/ol/claims/{claim_id}/` returns the claim header and policy context plus claimant, items, documents, internal file notes, requisition/payment link, loan-offset evidence, medical and assessment fields, financial summary, settlement/reinsurance evidence, permission-aware allowed actions, and a central audit timeline. The audit timeline includes claim-level events and claim-file-note events by readable claim-number representation, with actor display, timestamp, reason, source channel, and sanitized state snapshots.

`GET /api/v1/ol/claims/kpis/` applies the same filters as the register and computes real-time total claim count, outstanding amount, settled amount for the filtered period, and pending-assessment count. Amounts are formatted to two decimals. A single-currency result includes its currency code; mixed-currency results return `MULTI` plus a `currency_totals` breakdown so amounts are never silently combined across currencies. `GET /api/v1/ol/claims/export/` applies the same filters and returns a UTF-8 CSV with readable column headings and values, including policyholder name, status label, currency, and fraud flag.

## Partner portal, notifications, and discharge voucher

Prompt 11 adds a separate partner portal namespace at `/api/v1/portal/claims/`. `GET /api/v1/portal/claims/` and `GET /api/v1/portal/claims/{claim_number}/` are read-only and scope every query through the authenticated user’s current linked partner. A UUID may be accepted in the detail path for compatibility, but user-facing labels and responses use claim number, policy number, claimant name, and status labels. A partner outside the linked scope receives a sanitized `PORTAL_RESOURCE_NOT_FOUND` response rather than confirmation that another partner’s claim exists.

`POST /api/v1/portal/claims/register/` accepts only policy number, claim type, claim date, claim narrative, benefit type, member reference, and claimant details. It requires `X-Idempotency-Key`, resolves the policy within the linked partner scope, and delegates all eligibility, duplicate, waiting-period, claim-type, claimant, amount, audit, and event behavior to the same `ClaimRegistrationService` used by staff. Portal-created claims carry `source_channel=PORTAL` and preserve idempotent replay behavior.

The `ClaimRegistered`, `ClaimAssessed`, and `ClaimSettled` durable domain events are consumed by an idempotent claims notification receiver. Each event writes the existing policy notification outbox for available EMAIL and SMS recipients and mirrors an actionable dashboard notification for active users linked to the policyholder partner. Stable external keys prevent duplicate channel rows when an event is retried. Dashboard deep links use the human-readable claim number; UUIDs remain only in machine entity fields.

`DISCHARGE_VOUCHER` is registered in the shared Documents app and uses `documents/discharge_voucher.html`, the common branding and CSS system, `DocumentEngine`, `DocumentInstance`, and the standard short-lived signed download URL contract. Staff access is governed by `ol_claims.print`. The print wrapper is available at `POST /api/v1/ol/claims/{claim_id}/print-discharge-voucher/` and the generic shared endpoint remains `POST /api/v1/documents/render/DISCHARGE_VOUCHER/{claim_id}/`. The voucher includes claim, policy, policyholder, claimant, approved amount, loan offset, net payout, payment reference, and signature blocks. CANCELLED and REJECTED claims render with a visible status watermark; no other terminal status is presented as a watermark.

## Final seed scenarios and release verification

The final command `seed_ol_claim_scenarios --json` creates or reuses exactly ten claims with the `OL-CLAIM-SEED-` prefix. The scenarios cover full death settlement, partial critical-illness settlement, pending medical review, rejection for missing documents, rejection for the configured waiting period, duplicate-claim blocking, fraud-flagged assessment, settlement with policy-loan offset, waiver of premium, and cancellation. The loan scenario uses real OL Policies `PolicyLoan` and `PolicyLoanRepayment` records and is safe to rerun without a second offset.

The command captures four failure proofs as structured results: inactive policy returns `CLAIM_POLICY_INACTIVE`, duplicate registration returns `CLAIM_DUPLICATE`, a waiting-period date returns `CLAIM_WAITING_PERIOD_ACTIVE`, and an assessment above the calculated maximum returns `CLAIM_ASSESSMENT_AMOUNT_INVALID`. Each proof includes the expected code, returned code, human-readable message, and `passed=true`. A dedicated regression test runs the command twice in a fresh database and confirms ten claims, one offset record, all required lifecycle states, and stable idempotent output.

Prompt 12 release gates are `manage.py check`, migration-drift detection, Python compilation, the complete Claims foundation suite, the final scenario-seeder regression test, and the existing OL Policies integration suite. The authoritative prompt ledger is marked complete only after these checks pass and the release commit is pushed to `origin/sultan`.
